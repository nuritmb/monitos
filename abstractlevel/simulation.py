...
class Simulation:
    ...
    def __init__(
            ...
            signal_list: List[MonkeySignal], state_list: List[MonkeyState], delete_only_elderly: bool=True,
            archive_cicle: int=100, min_monkeys: int=1, archive_maps: bool=False) -> None:
        # Received parameters
    ...
    def replication_phase(self, starting_number: Union[int,None]=None) -> None:
        ...
        if (self.turn % self.archive_cicle) == 0:
            ...
            self.archives[-1]['Replication Phase Time (Archiving)'] = time.time() - t2
    ...
    def run(self, nturns: int) -> None:
        ...
        self.create_monkeys()
        t1 = time.time()
        print('Game started ({time:.4f}s, {timepm:.0f}μs/monkey).'.format(time=(t1-t0),timepm=1000000*(t1-t0)/self.nmonkeys))
        for i in range(1, nturns+1):
            if len(self.monkey_list) < self.min_monkeys:
                t1 = time.time()
                gameduration = t1 - t0
                print('There are less than {nmonk:d} monkeys. Game ended in {nturns:d} turns. Duration: {dur:.4f} ({durpt:.0f} ms per turn)'.format(
                    nmonk=self.min_monkeys,
                    nturns=i,
                    dur=gameduration,
                    durpt=1000*gameduration/i))
                return
            self.run_turn()
        t1 = time.time()
        gameduration = t1 - t0
        print('All turns {nturns:d} have passed. Game ended with {nmonk:d} monkeys. Duration: {dur:.4f} ({durpt:.0f} ms per turn)'.format(
            nturns=i,
            nmonk=len(self.monkey_list),
            dur=gameduration,
            durpt=1000*gameduration/i))